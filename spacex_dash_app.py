...
launchNames = spacex_df.groupby('Launch Site', as_index=False).first()[['Launch Site']]
opts=[ {'label': 'All Sites', 'value': 'ALL'}, ]
for id, row in launchNames.iterrows():
    opts.append({'label': row['Launch Site'], 'value': row['Launch Site']})
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        filtered_df['Type'] = np.where(filtered_df.index == 0, 'Success', 'Failure')
...
def get_scattered_plot(entered_site, entered_mass):
